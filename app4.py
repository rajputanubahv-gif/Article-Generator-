...
# ========================
def generate_audio_summary(text, language='en'):
    """Generate audio summary from text using gTTS"""
    try:
        # Create a shorter summary for audio (limit to 500 characters for demo)
        if len(text) > 500:
            summary_text = text[:497] + "..."
        else:
            summary_text = text
...
class DraftManager:
    ...
    def _to_pdf(self, title, content):
        """Generate a well-structured PDF with proper formatting"""
        buffer = BytesIO()
        ...
        if not drafts:
            ...
            for i, d in enumerate(drafts):
                ...
                with st.sidebar.expander(expander_title):
                    ...
                    if st.button("🔊 Audio Summary", key=f"audio_{i}"):
                        with st.spinner("Generating audio summary..."):
                            audio_buffer = generate_audio_summary(d["content"])
                            if audio_buffer:
                                st.audio(audio_buffer, format="audio/mp3")
                                st.download_button(
                                    "⬇️ Download Audio", 
                                    audio_buffer, 
                                    file_name=f"{d['title']}_summary.mp3",
                                    key=f"audio_dl_{i}"
                                )
...
class ToolExecutor:
    ...
    def execute(self, intent: str, params: dict, save_draft=True):
        ...
        if intent == "SUMMARIZE":
            ...
            # First generate a summary, then convert to audio
            rag_chain = build_rag_chain(content)
            summary = rag_chain.run("Create a concise 2-3 paragraph summary suitable for audio presentation.")
            return summary, f"Audio Summary of '{title}'"
...
def get_intent(user_prompt: str) -> dict:
    llm = GeminiLLM()
    tools_list = "[GENERATE_ARTICLE, SUMMARIZE, QUESTION_ANSWERING, KEYWORD_EXTRACTION, READABILITY_ANALYSIS, TRANSLATE, REPHRASE, SOCIAL_MEDIA, PRESENTATION_OUTLINE, FURTHER_READING, AUDIO_SUMMARY, GENERAL_CHAT]"
    router_prompt = f"""
    ...
    - If the user asks for an audio summary or spoken version, use `AUDIO_SUMMARY`.
    ...
    - "create an audio summary" -> {{"intent": "AUDIO_SUMMARY", "parameters": {{}}}}
...
tab_titles = [
    ...
    "**🌐 Translate**", "**🔊 Audio Summary**", "**📂 Manage Drafts**"
]
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                if intent in content_dependent_intents and not active_content:
                    ...
                    # Handle audio summary specially
                    if intent == "AUDIO_SUMMARY":
                        audio_buffer = generate_audio_summary(content)
                        if audio_buffer:
                            st.audio(audio_buffer, format="audio/mp3")
                            st.download_button(
                                "⬇️ Download Audio Summary", 
                                audio_buffer, 
                                file_name=f"{title_for_rerun}.mp3",
                                key="audio_download_chat"
                            )
                        response = f"**Audio Summary Generated**\n\nHere's a text version of the summary:\n\n{content}"
                    # Handle other intents normally
                    elif intent == "GENERATE_ARTICLE" and title_for_rerun:
...
# --- Tab 8: Audio Summary ---
with tabs[7]:
    st.header("Create Audio Summary")
    selected_draft = get_draft_selector("audio_tab")
    if selected_draft:
        st.info("Generate an audio summary of your article. Perfect for listening on the go!")
        ...
        selected_lang_name = st.selectbox("Select language for audio:", list(audio_languages.keys()))
        lang_code = audio_languages[selected_lang_name]
        
        if st.button("🔊 Generate Audio Summary", key="audio_summary_btn"):
            with st.spinner("Creating audio summary..."):
                params = {
                    "article_content": selected_draft['content'], 
                    "article_title": selected_draft['title'],
                    "language": lang_code
                }
                summary_text, title = tool_executor.execute("AUDIO_SUMMARY", params, save_draft=False)
                ...
                audio_buffer = generate_audio_summary(summary_text, lang_code)
                if audio_buffer:
                    ...
                    st.download_button(
                        "⬇️ Download Audio Summary", 
                        audio_buffer, 
                        file_name=f"{selected_draft['title']}_audio_summary.mp3",
                        key="audio_download_tab"
                    )
